...
def _status_from_http(status_code: int, response_time_ms: int, slow_threshold_ms: int) -> CheckRunStatus:
    if status_code >= 400:
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            async with httpx.AsyncClient(follow_redirects=True) as client:
                for attempt in range(monitor.max_retries + 1):
                    retry_count = attempt
                    started_at = datetime.now(UTC)
                    try:
                    ...
                    if not _is_retryable_status(run_status) or attempt >= monitor.max_retries:
                        break
            ...
            if run_status == CheckRunStatus.UP:
                monitor.current_status = MonitorStatus.UP
                monitor.last_success_at = finished_at
            elif run_status == CheckRunStatus.SLOW:
                monitor.current_status = MonitorStatus.SLOW
                monitor.last_success_at = finished_at
            else:
            ...
            # UP/SLOW => close active incident if any
            ...
            # DOWN => open incident if not already open
            else:
